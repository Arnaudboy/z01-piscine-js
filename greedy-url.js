function getURL(data) {return data.match(/http(s?):\/\/\w+.\S+/g)}

function greedyQuery(data) {
    let returnarr = []
    getURL(data).forEach(elem => elem.match(/=/g) != null && elem.match(/=/g).length >= 3 ? returnarr.push(elem) : elem)
    return returnarr
}

function notSoGreedy(data) {
    let returnarr = []
    getURL(data).forEach(elem => elem.match(/=/g) != null && elem.match(/=/g).length <= 3 && elem.match(/=/g).length >= 2 ? returnarr.push(elem) : elem)
    return returnarr
}
...
// //console.log(getURL(dataSet), $getURL)
// console.log(greedyQuery(dataSet), $greedyQuery)
// console.log(notSoGreedy(dataSet), $notSoGreedy)